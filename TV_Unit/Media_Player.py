...
# V5 - Intergrate pyqt5 with IR commands - working but slow, best combo is using threads for everything!
...
class Ui_video_screen(QWidget):
    ...
    def __init__(self, video_stream_address, gpio_input, main_screen_class):
        ...
        self.q_thread = thread_queue(maxsize = 1)
        self.vid_thread = threading.Thread(target = self.start_vid)
    ...
    def initUI(self):
        ...
        self.q_thread = thread_queue(maxsize = 1)
        #self.vid_core = threading.Process(target = self.start_vid)
        self.vid_thread = threading.Thread(target=self.start_vid)
    ...
    def start_vid(self):    #issue with mpv and ir reader. When both run at same time the thread is shared between the ir and mpv meaning that the remote doesn't detect a command as 1 whole command
        ...
        self.q_thread.get() #method will wait here until a user inputs something into q_core meaning the video will play here
        self.player.stop()
